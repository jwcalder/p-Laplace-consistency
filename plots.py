...
def imsave(fname, img, scale=1, gray=True, order=0):
    if scale != 1:
        if img.ndim == 2:
            ...
            rimg = rescale(img, (scale,scale,1), order=order)
    else:
        rimg = img
...
def plot_region(X,L,clf,alpha=0.5,cmap='Paired',cp=np.array([0.5,2.5,6.5,8.5,10.5,11.5]),markers = ['o','s','D','^','v','p'],vmin=0,vmax=12,fname=None,markersize=75,linewidths=1.25,markerlinewidths=1,res=0.01,train_pts=None,red_pts=None,unlabeled_size_factor=0.5):
    ...
    #Scatterplot points from each class
    for i,color in enumerate(c_u):
        ...
        else: #Plot labeled and unlabeled points differently
            plt.scatter(x[sub & ~train_pts],y[sub & ~train_pts],zorder=2,c='black',marker='o',s=unlabeled_size_factor*markersize)
            plt.scatter(x[sub & train_pts],y[sub & train_pts],zorder=2,c=c[sub & train_pts],cmap=cmap,edgecolors='black',vmin=vmin,vmax=vmax,linewidths=markerlinewidths,marker=markers[i],s=markersize)
        if red_pts is not None:
            plt.scatter(x[sub & red_pts],y[sub & red_pts],zorder=2,c=np.ones(np.sum(sub&red_pts))*5.5,cmap=cmap,edgecolors='black',vmin=vmin,vmax=vmax,linewidths=markerlinewidths,marker=markers[i],s=2*markersize)

    plt.xlim((xmin,xmax))
    plt.ylim((ymin,ymax))
    if fname is not None:
        ext = fname[-4:]
        savefig(fname[:-4] + '_preDF'+ext,axis=True,pad_inches=0.01)
